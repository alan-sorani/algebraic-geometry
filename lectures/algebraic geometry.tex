...
Let $f \in K^\times$, we define \stress{the principal divisor} \[\mrm{div}\prs{f} = \sum_Y v_Y\prs{f} Y \in \mrm{Div} X \text{.}\]
\end{definition}
...
In this case say $D,D'$ are \stress{linearly equivalent}.
\end{definition}

\begin{definition}[Divisor Class Group]
Define \[\mrm{Cl}\prs{X} \ceq \quot{\mrm{Div}\prs{X}}{\set{\mrm{div}\prs{f}}{f \in K}} \text{.}\]
This is the \stress{divisor class group of $X$}.
\end{definition}

\begin{definition}[Normal Scheme]
A scheme $X$ is \stress{normal} if all local rings $\OOO_{X,x}$ are integrally-closed.
\end{definition}
...
\begin{lemma}[Matsumura, Thm 38] \label{lemma:matsumura}
...
\begin{definition}[Degree of a Divisor in $\PP_k^n$]
